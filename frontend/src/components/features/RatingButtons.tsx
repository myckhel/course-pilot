...
interface RatingButtonsProps {
  message: ChatMessage;
  onRating: (messageId: string, rating: "positive" | "negative" | null) => Promise<void>;
  disabled?: boolean;
}

function RatingButtons({ message, onRating, disabled = false }: RatingButtonsProps) {
  ...
  return (
    <Space size="small" className="mt-2">
      <Tooltip title={message.rating === "positive" ? "Remove positive rating" : "Mark as helpful"}>
        <Button
          type="text"
          size="small"
          icon={message.rating === "positive" ? <LikeFilled /> : <LikeOutlined />}
      ...
      <Tooltip title={message.rating === "negative" ? "Remove negative rating" : "Mark as not helpful"}>
        <Button
          type="text"
          size="small"
          icon={message.rating === "negative" ? <DislikeFilled /> : <DislikeOutlined />}
